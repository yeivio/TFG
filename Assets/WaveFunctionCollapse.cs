...
public class WaveFunctionCollapse : GenerationAlgorithm
{
    ...
    public void SpawnTile(int x, int y)
    {
        ...
        try
        {
            aux.AddComponent<SpriteRenderer>().sprite = map[x, y].options[0].GetComponent<SpriteRenderer>().sprite;
        }
        catch (Exception ex)
        {
            Debug.Log("Seed:" + seed);
    ...
    private void CleanMap()
    {
        ...
        for(int i = 0; i < widthMap; i++) {
            for (int j = 0; j < heightMap; j++)
            {
                map[i,j] = new Cell(usableTiles, i, j);
            }
        }
    }
    ...
    {
        // Update bottom cell
        if( cell.yPos > 0 && !map[cell.xPos, cell.yPos -1 ].collapsed)
        {
            bool valid;
            foreach(Tiles tilOption in new List<Tiles>(map[cell.xPos, cell.yPos - 1].options))
            {
                valid = false;
                ...
                if (!valid)
                    map[cell.xPos, cell.yPos - 1].options.Remove(tilOption);
            }
        }


        // Update LeftCell
        if (cell.xPos > 0 && !map[cell.xPos-1, cell.yPos].collapsed)
        {
            bool valid;
            foreach (Tiles tilOption in new List<Tiles>(map[cell.xPos-1, cell.yPos].options))
            {
                valid = false;
                ...
                if (!valid)
                    map[cell.xPos-1, cell.yPos].options.Remove(tilOption);
            }
        }
        ...
        // Update Top cell
        if (cell.yPos < heightMap - 1 && !map[cell.xPos, cell.yPos + 1].collapsed)
        {
            bool valid;
            foreach (Tiles tilOption in new List<Tiles>(map[cell.xPos, cell.yPos + 1].options))
            {
        ...
        // Update RightCell
        if (cell.xPos < widthMap - 1  && !map[cell.xPos + 1, cell.yPos].collapsed)
        {
            bool valid;
            foreach (Tiles tilOption in new List<Tiles>(map[cell.xPos + 1, cell.yPos].options))
            {
